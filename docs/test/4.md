...
![image-20221113144256984](../assets/image-20221113144256984.png)
...
it('input can be typed', () => {
  ...
  expect(
    screen.getByDisplayValue('type some text continue'),
  ).toBeInTheDocument();
});
```
...
it('be change when user click add button and x button', async () => {
  ...
  expect(onChange).toHaveBeenCalledWith([
    { id: expect.any(Number), isComplete: false, text: 'item1' },
  ]);
