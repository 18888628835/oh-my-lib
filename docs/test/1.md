...
![image-20221101212845676](../assets/image-20221101212845676.png)
...
```bash
npm i -D ts-jest @types/jest ts-node typescript
```

- 安装 ts-jest 、jest 类型声明。ts-jest 用来转译 TypeScript 的代码
...
1. 初始化了一些环境配置
2. 要让 Jest 支持 TypeScript，需要用到转译器，主流的转译器有两个：tsc 和 babel。tsc 能让 Jest 支持类型检查，所以更推荐使用
3. 能够通过路径别名来让 `Jest` 识别路径映射。除了在 `tsconfig` 中写路径别名外，还需要在 `jest.config.js` 中配置`moduleDirectories` 和 `moduleNameMapper`来实现
...
jest 提供了一个 mock 的 fn 函数， fn 函数可以被捕获到是否执行过。
...
前端开发中经常用 mock 函数来模拟请求是否发送了。

例如，模拟 axios 的请求发送了（但不在乎结果）
...
我们先用 `jest.mock('axios')`mock 了 axios。

接着用`mockedAxios.get.mockResolvedValue`来 mock 出结果为`{ data: 'anything' }`。

最后，我们断言结果为`{ data: 'anything' }`表示`axios.get`请求确实发送了。

这种行为并不会真实发送 axios.get 请求，同时也不在乎返回结果。在不测试后端结果的场景下非常节省时间。
